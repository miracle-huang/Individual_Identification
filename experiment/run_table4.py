...
def run_table4():
    ...
    for video in video_list_2:
        ...
        chunk_dict_train = load_all_data(
                    config.subject_amount_30, video_list_1d, window_size = window_size, sliding_size = sliding_size)
        print('test:')
        chunk_dict_test = load_all_data(
            config.subject_amount_30, video, window_size = window_size, sliding_size = sliding_size)
        
        save_folder = f'result/table4/number1train_number2test_30/{video[0]}/'
    ...
    #     save_folder = f'result/table4/number2train_number1test_30/{video[0]}/'
    #     if not os.path.exists(save_folder):
    #         os.makedirs(save_folder)
    #     xlsx_name = os.path.join(save_folder, f'window_size{window_size}_sliding_size{sliding_size}.xlsx')
    #     wb = Workbook()
